...
class ClientOrderThrottler(Process):
    ...
    async def async_init(self):
        ...
        try:
            ...
            self.mongo_db_async = self.mongo_client_async['symphonyorder_raw'][
                f'orders_{str(date.today())}']
    ...
    def _add_proxy(gateway, client_id) -> str:
        if not gateway or gateway in ['null', 'None']:
            return None
        else: return f'http://{client_id}:{client_id}@{gateway}'
    ...
    async def send_order_as_http_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                tokenMap = json.loads(await self.redis_token_async.get('tokenMap'))
            except:
            ...
            if token['type'].lower() == 'pro':
                final_order['clientID'] = '*****'

            if 'endpoint' in tokenMap[order['clientID']]:
                endpoint = tokenMap[order['clientID']]['endpoint']
                suffix = url.split('interactive')[-1]
            ...
            async with self.http_session.request(
                ...
                json=final_order,
                headers=headers,
                proxy=self._add_proxy(token.get('gateway'), order['clientID'])
            ) as response:
                ...
                if response.status != 200:
                    ...
                    error_packet = {'message': order_text, 'order_data': json.dumps(order)}
                    await self.redis_executor_infra_async.xadd(self.error_stream, error_packet)
        ...
        try:
            ...
            async with self.http_session.request(
                ...
                if response.status != 200:
                    ...
                    error_packet = {'message': order_text, 'pending_order': json.dumps(order)}
                    await self.redis_executor_infra_async.xadd(self.error_stream, error_packet)
