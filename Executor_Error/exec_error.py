...
class ExecErrorProcessor:
    ...
    async def initialize(self):
        ...
        self.orders_db = self.db_mongo_client['algo_signals'][f'algo_signals_{datetime.date.today().strftime("%Y-%m-%d")}']
        self.response_db = self.db_mongo_client['Info']['final_response']
    ...
    async def process_error(self, message_id: str, error_data: dict):
        ...
        async with self.processing_semaphore:
            try:
                ...
                if 'order_data' == error_data['error_type']:
                    message = error_data['reason']
                    order_data = json.loads(error_data['signal'])
                ...
                elif 'pending_order' == error_data['error_type']:
                    message = error_data['reason'] 
                    pending_data = json.loads(error_data['signal'])
                    ...
                    await self.logger.info(f"Processed error: {message} for pending order: {pending_data}")
                ...
                elif error_data['error_type'] == 'AlgoSignalValidationError':
                        message = error_data['reason']
                        order_data:dict = json.loads(error_data['signal'])
                        ...
                        print(order_data)
                        print(client_action_map)
                        if client_action_map:
                            ...
                            await self.logger.warning(f"client_action_map empty for algo {order_data['algoName']}, falling back to MongoDB")
                            cursor = self.db_mongo_client['Info']['client_rms_params'].find(
                                {'algo_name': order_data['algoName']},
                                {'client_id': 1}
                            )
    ...
    async def send_alarm(self, order_data: dict, message: str, alarm:str = 'reject_rms'):
        ...
        try:
            ...
            await self.ping_redis.rpush('alarms-v2', 6)
